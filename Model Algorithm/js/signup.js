...
function formValidate(){
	...
	if(!isEmpty(fname,"First Name",156,112,-27,2))
		if(fNameValidation())
			if(!isEmpty(lname,"Last Name",156,112,-27,48))
				if(lNameValidation())
					if(!isEmpty(address,"Address",140,88,-25,92))
						if(addressValidation())
							if(!isEmpty(bday,"Date of Birth",158,106,-100,240))
								if(isNull())
									if(!isEmpty(email,"Email",128,75,-27,285))
										if(emailValidation())
											if(!isEmpty(uname,"Username",152,96,-27,329))
												if(unameValidation())
													if(!isEmpty(password,"Password",156,112,-27,374))
...
function addressValidation(){
	...
	if(!address==""||!address==null){
		if(address.length>=10&&address.length<=150)
			...
			document.getElementById("popup3").style.marginLeft=-25+"px";
...
function emailValidation(){
	...
	if(!email==""||!email==null){
		...
		if(atpos<1||dotpos+2>=email.length||atpos+2>dotpos){
			...
			document.getElementById("popup5").style.marginLeft=-27+"px";
...
function checkValidation(){
	var check=document.form1.check.checked;
	if(check)
		return true;
	else{
		document.getElementById("popup9").style.marginTop=527+"px";
		document.getElementById("popup9").style.marginLeft=-94+"px";
		document.getElementById("popup9").style.display="block";
		document.getElementById("popup9").style.width=306+"px";
		document.getElementById("msg9").style.width=250+"px";
		document.getElementById("msg9").innerHTML="You must agree with our terms and conditions";
		return false;
	}
}